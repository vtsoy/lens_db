...
COMMENT ON table cities is ' Города доставки';
...
CREATE TABLE shipping_info (
    ...
    free_from numeric,
    delivery_period character varying,
    description character varying
);
COMMENT ON TABLE shipping_info IS 'Информация об условиях доставки';
COMMENT ON COLUMN shipping_info.free_from IS 'Стоимость заказа, с которого доставка бесплатно';
COMMENT ON COLUMN shipping_info.delivery_period IS 'Срок доставки';
COMMENT ON COLUMN shipping_info.description IS 'Курьерская служба? Ну или можно убрать поле';
...
COMMENT ON column orders.customer_id IS 'Nullable потому что, заказ может быть еще не сформирован, а значит никем не взят';
COMMENT ON column orders.city_id is 'Город доставки';
COMMENT ON column orders.street is 'Улица';
COMMENT ON column orders.building is 'Дом';
COMMENT ON column orders.apartment is 'Квартира';
COMMENT ON column orders.comment is 'Комментарий к заказу';
COMMENT ON column orders.shipping_price IS 'Стоимость доставки';
